...
const labels = ["Area", "TPU", "Gloss", "Warranty"];
const specKeys = ["area", "tpu", "gloss", "warranty"];
...
export default function PPFOptions() {
  ...
  const [activeCol, setActiveCol] = useState(3); // highlight PLATINUM by default

  // Booking modal + flow (same pattern as other option components)
  ...
  return (
    ...
    >
      <style jsx>{`
        ...
        .ppf-table-grid {
          display: grid;
          grid-template-columns: 160px repeat(6, 132px);
          gap: 18px;
          width: 1224px;
          margin: 0 auto;
          align-items: start;
        ...
        .ppf-package-column:hover::before,
        .ppf-package-column.active::before {
          background: linear-gradient(
            135.31deg,
            #9e8976 15.43%,
            #7a5e50 30.62%,
        ...
        .ppf-package-column:hover,
        .ppf-package-column.active {
        ...
        .ppf-package-value {
          width: 100%;
          /* follow spec: vertical padding 18px */
          padding: 16px 0;
          display: flex;
          align-items: center;
          justify-content: center;
        ...
        .ppf-package-column:hover .ppf-book-btn,
        .ppf-package-column.active .ppf-book-btn {
          border-radius: 999px;
          background: linear-gradient(
            114.31deg,
            #151000 -20.74%,
        ...
        /* Hide mobile card list on desktop — only show inside media query */
        .ppf-table-mobile {
          display: none;
        }
        ...
        @media (max-width: 640px) {
          ...
          /* Reduce outer container + inner padding on mobile */
          .ppf-table-container {
          ...
          /* hide decorative glows on small screens */
          .ppf-glow-tl,
          .ppf-glow-br {
            display: none !important;
          }

          .ppf-table-mobile .mobile-card {
            background: linear-gradient(
              180deg,
              rgba(255, 255, 255, 0.02),
              rgba(255, 255, 255, 0)
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-[112px] ppf-table-container">
        <div
          style={{
            marginBottom: "60px",
            display: "flex",
        ...
        >
          ...
          />
          <h2 className="explore-header text-[32px] leading-[40px] md:text-[48px] md:leading-[48px]"
            style={{
              position: "relative",
              zIndex: 1,
          ...
          >
            Explore Our PPF Options
          </h2>
        </div>

        <div className="ppf-table-wrapper" style={{ position: "relative" }}>
          {/* Top-left eclipse glow */}
          <div
            className="ppf-glow-tl"
            style={{
              position: "absolute",
              left: "-60px",
              top: "-60px",
          ...
          <div
            className="ppf-table-inner"
            style={{ overflow: "visible", position: "relative", zIndex: 2 }}
          >
            <div className="ppf-table-grid">
              <div className="ppf-labels-column">
                <div className="header-space">
                  <div className="ppf-header-text ml-4 mt-6">Package</div>
                </div>
                {labels.map((lbl, li) => (
                  <div className="ppf-label" key={li}>
                    <span className="ppf-label-text">{lbl}</span>
                    {/* show small ◎ icon for Area, TPU, Gloss */}
                    {["Area", "TPU", "Gloss"].includes(lbl) && (
                      <span className="dot">◎</span>
                    )}
                  </div>
                ))}
                <div className="ppf-label">
                  <div className="ppf-header-text mt-2">Price</div>
                </div>
              </div>
              ...
              {packages.map((pkg, i) => (
                <div
                  key={i}
                  className={`ppf-package-column ${activeCol === i ? "active" : ""}`}
                  onMouseEnter={() => setActiveCol(i)}
                  onFocus={() => setActiveCol(i)}
                  tabIndex={0}
                >
                  <div className="ppf-package-header">
                    <span className="ppf-header-text">{pkg.name}</span>
                  </div>

                  <div className="ppf-package-value">{pkg.area}</div>
                  <div className="ppf-package-value">{pkg.tpu}</div>
                  <div className="ppf-package-value">{pkg.gloss}</div>
                  <div className="ppf-package-value">{pkg.warranty}</div>

                  <div className="ppf-price">
                    <span className="ppf-price-current">{pkg.price}</span>
                    <span className="ppf-price-original">{pkg.original}</span>
                  </div>

                  <div className="package-cta">
                    <button
                      onClick={() => handleBookClick(pkg)}
                      className={`ppf-book-btn ${i < 4 ? "book-top" : "book-bottom"}`}
                    >
                      Book
                    </button>
                  </div>
        ...
        <div className="mobile-scroll-wrapper relative md:hidden">
          ...
          <div className="labels-mobile">
            ...
            {labels.map((lbl, li) => (
              <div className="mobile-label" key={li}>
                ...
                {['Area','TPU','Gloss'].includes(lbl) && <span className="dot">◎</span>}
              </div>
            ))}
            <div className="mobile-label">
          ...
          <div className="scroll-content flex ml-[120px]">
            <div className="ppf-table-mobile" ref={mobileTableRef}>
              {packages.map((pkg, i) => (
                <div className="mobile-card" key={i}>
                  ...
                  <div className="mobile-value">{pkg.area}</div>
                  <div className="mobile-value">{pkg.tpu}</div>
                  <div className="mobile-value">{pkg.gloss}</div>
                  <div className="mobile-value">{pkg.warranty}</div>

                  <div className="mobile-price">
                    <div className="ppf-price-current">{pkg.price}</div>
                    <div className="ppf-price-original">{pkg.original}</div>
                  </div>

                  <div className="mobile-cta">
                    <button
                      className="ppf-book-btn"
                      onClick={() => handleBookClick(pkg)}
                    >
                      Book
                    </button>
      ...
      <BookingModal
        ...
        packageData={{
          ...
          tpu: selectedPackage?.tpu,
          gloss: selectedPackage?.gloss,
